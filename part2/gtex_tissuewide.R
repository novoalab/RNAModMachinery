...
pdf("heatmap.gtex.logzscaled.pdf",height=12,width=8)
...
print(ggplot(data_out,aes(x=PC1,y=PC2,color=Class,label=Symbol))+
	...
	geom_vline(xintercept = 0, lty = 2)+
	geom_point(alpha = 0.8, size = 1.2)+
	geom_text(data=subset(data_out, PC1 > 2 | PC1 < -2| PC2 > 2 | PC2 < -2),
...
pdf(file="pca_gtex.loadings.all.labels.pdf",height=5,width=5)
...
data_out_r <- as.data.frame(data_pca$rotation) #rotation data (loadings)
data_out_r$Symbol <- row.names(data_out_r) 
pdf(file="pca_gtex.loadings.some.labels.pdf",height=5,width=5)
print(ggplot(data_out_r,aes(x=PC1,y=PC2,label=Symbol))+
	geom_point()+
	geom_hline(yintercept = 0, lty = 2) +
	geom_vline(xintercept = 0, lty = 2)+
